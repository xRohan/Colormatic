...
import com.pugz.colormatic.core.registry.ColormaticItems;
...
public class ColormaticBoatEntity extends BoatEntity {
    private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(ColormaticBoatEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(ColormaticBoatEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(ColormaticBoatEntity.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(ColormaticBoatEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean> field_199704_e = EntityDataManager.createKey(ColormaticBoatEntity.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> field_199705_f = EntityDataManager.createKey(ColormaticBoatEntity.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(ColormaticBoatEntity.class, DataSerializers.VARINT);
    ...
    public ColormaticBoatEntity(EntityType<? extends BoatEntity> type, World world) {
        super(type, world);
        preventEntitySpawning = true;
    }

    public ColormaticBoatEntity(World worldIn, double x, double y, double z) {
        super(ColormaticEntities.WISTERIA_BOAT, worldIn);
        setPosition(x, y, z);
    ...
    public ColormaticBoatEntity(FMLPlayMessages.SpawnEntity spawnEntity, World world) {
        this(ColormaticEntities.WISTERIA_BOAT, world);
    }
    ...
    public Item getItemBoat() {
        switch (getBoatModel()) {
            case WISTERIA:
            default:
                return ColormaticItems.WISTERIA_BOAT;
        }
    }
        ...
        if (!isPassenger()) {
            if (onGroundIn) {
                if (fallDistance > 3.0F) {
                    if (status != ColormaticBoatEntity.Status.ON_LAND) {
                        fallDistance = 0.0F;
                        return;
                    }
    ...
    @Override
    @OnlyIn(Dist.CLIENT)
    public void updateInputs(boolean left, boolean right, boolean forward, boolean backward) {
        leftInputDown = left;
        rightInputDown = right;
        forwardInputDown = forward;
        backInputDown = backward;
    }
    ...
    public enum Type {
        WISTERIA(ColormaticBlocks.WISTERIA_PLANKS, "wisteria");

        private final String name;
        private final Block block;

        Type(Block blockIn, String nameIn) {
            name = nameIn;
            block = blockIn;
        }
